...
open(DEBUG,">/export/Shell/DEBUG")|| print "cannot open /export/Shell/DEBUG\n";

if (!$todo || ($allorthismonth ne "all" && $allorthismonth ne "this" ) ) {
	print "Usage:\n\n" ;
	print "  ./calculateLogs.pl  company  type\n\n" ;
	print "company = export directory name or \"all\" for all companies\n" ;
	print "type    = \"all\" for all months data or \"this\" for this month only\n\n" ;
	exit ;
}
...
#				print $thisdate." ".$thissubdiv." '".$separate."' ".$separatesubdiv{$separate}."\n" ;
				...
				#writedatabase!
				$_ = $thisdate ;
				($YEAR,$MONTH,$DAY) = /(\d*)_(\d*)_(\d*)/ ;
				$orphanId = &insert(	"analysis_counts",
							"day,month,year,subdivision,count",
							$DAY.",".$MONTH.",".$YEAR.",'".$thissubdiv."',".$separatesubdiv{$separate}  ) ;
...
#				close(DUMP) ;
				...
				# All subdiv directories should be open...
